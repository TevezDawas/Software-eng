...
//		/////////////////////////////////
	...
	// i.e. player wins the match if player wins this Model.
	 ...
	 * announces Model over on infoPnl and dialog prompt,
	 * then ask if player wants another Model.
	 * 
	...
	public void handleGameOver(boolean isIntermediate) {
		...
		if (root.isMatchOver())
			...
			Dialogs<ButtonType> dialog = new Dialogs<ButtonType>("Winner Winner Chicken Dinner! " + pCurrent.getShortName() + " wins! " + playerResult, stage, "Next Model");
			...
			if (result.get().equals(dialog.getButton())) {
				...
				infoPnl.print("Enter /start if you wish to resume the Model.", MessageType.ANNOUNCEMENT);
	 ...
	 * @return boolean value indicating if Model is over.
	 */
