...
public class EventController implements ColorParser, ColorPerspectiveParser, InputValidator {
	...
	private void initGameListeners() {
		// Exit selection mode when any part of the Model board is clicked.
	...
	TouchableHandler touchableHandler = new TouchableHandler() {
		...
		public void onClicked(Touchable object) {
			if (object instanceof DoublingCube) {
				...
				// if cube at its box.
				if (!Model.getOtherHome().getCubeHome().isEmpty()) {
				...
				if (gameplay.isStarted()) {
					...
					if (!gameplay.isDoubling()) {
						// highlight cube box.
						Model.getOtherHome().getCubeHome().highlight();
					}
					
					Model.getOtherHome().getHome(gameplay.getCurrent().getColor()).highlight();
				} else {
					// highlight player's other homes.
					Model.getOtherHome().getHome(Settings.getBottomPerspectiveColor()).highlight();
					Model.getOtherHome().getHome(Settings.getTopPerspectiveColor()).highlight();
				}
	...
	*/
	...
	private void initStageListeners() {
		// checks if player really wants to exit Model prevents accidental exits
		stage.setOnCloseRequest((WindowEvent event) -> {
			...
			infoPnl.print("Trying to quit Model.");
			cmd.runSaveCommand();
